...
async function autoUpdate(ProID) {
    ...
    if(top2.length < 2){
        ...
        return await db('bid_history').insert({'BID': top2[0].BID, 'Price': parseInt(start_price[0].Price) + step, 'ProID': ProID, 'Time': today})
    }
    ...
    //update bid history
    try {
        await db('bid_history').insert({'BID': top2[1].BID, 'Price': parseInt(top2[1].MaxPrice), 'ProID': ProID, 'Time': today})
    } catch (error) {
        await db('bid_history').where({'BID': top2[1].BID, 'ProID': ProID}).update({'Price': parseInt(top2[1].MaxPrice), 'Time': today});
    }
    try {
        await db('bid_history').insert({'BID': top2[0].BID, 'Price': parseInt(top2[1].MaxPrice) + step, 'ProID': ProID, 'Time': today})
    } catch (error) {
        await db('bid_history').where({'BID': top2[0].BID, 'ProID': ProID}).update({'Price': parseInt(top2[1].MaxPrice) + step, 'Time': today});
    }
}
export default {
    getHistoryByPro(id){
